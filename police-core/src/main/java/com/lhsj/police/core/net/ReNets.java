...
import com.lhsj.police.core.base.SystemProperties;
...
@Beta
public class ReNets {
    private static Logger logger = LoggerFactory.getLogger(ReNets.class);
    ...
    private static class LocalAddress {
        ...
        public LocalAddress() {
            ...
            hostName = RePlatforms.IS_WINDOWS ? System.getenv("COMPUTERNAME") : System.getenv("HOSTNAME");
        }
        ...
        private void initLocalAddress() {
            ...
            localHost = ReIPs.toIpString(localInetAddress);
        ...
        private static InetAddress findLocalAddressViaNetworkInterface() {
            ...
            String preferNamePrefix = SystemProperties.getString("localhost.prefer.nic.prefix",
                    "LOCALHOST_PREFER_NIC_PREFIX", "bond0.");
            // 如果hostname +/etc/hosts 得到的是127.0.0.1, 和首选网卡都不符合要求，则按顺序遍历下面的网卡
            String defaultNicList = SystemProperties.getString("localhost.default.nic.list",
                    "LOCALHOST_DEFAULT_NIC_LIST", "bond0,eth0,em0,br0,en0");

            InetAddress resultAddress = null;
            Map<String, NetworkInterface> candidateInterfaces = ReMaps.newHashMap();
