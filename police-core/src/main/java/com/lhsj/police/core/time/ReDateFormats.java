...
import com.lhsj.police.core.annotation.NotNull;
...
public class ReDateFormats {
    ...
    public static Date parseDate(@NotNull String dateString) throws ParseException {
        return FastDateFormat.getInstance(PATTERN_ISO_ON_DATE).parse(dateString);
    }

    public static Date parseDateTime(@NotNull String dateString) throws ParseException {
        return FastDateFormat.getInstance(PATTERN_DEFAULT_ON_SECOND).parse(dateString);
    }

    public static Date parseDateTimeMillis(@NotNull String dateString) throws ParseException {
        return FastDateFormat.getInstance(PATTERN_DEFAULT).parse(dateString);
    }
    ...
    public static String formatDateTimeMillis(@NotNull Date date) {
        return FastDateFormat.getInstance(PATTERN_DEFAULT).format(date);
    }
    ...
    public static String formatFriendlyTimeSpanByNow(long timeStampMillis) {
        long now = ReClocks.currentTimeMillis();
        long span = now - timeStampMillis;
        if (span < 0) {
            // 'c' 日期和时间，被格式化为 "%ta %tb %td %tT %tZ %tY"，例如 "Sun Jul 20 16:17:00 EDT 1969"。
            return String.format("%tc", timeStampMillis);
        }
        if (span < ReDates.MILLIS_PER_SECOND) {
            return "刚刚";
        } else if (span < ReDates.MILLIS_PER_MINUTE) {
            return String.format("%d秒前", span / ReDates.MILLIS_PER_SECOND);
        } else if (span < ReDates.MILLIS_PER_HOUR) {
            return String.format("%d分钟前", span / ReDates.MILLIS_PER_MINUTE);
        }
        // 获取当天00:00
        long wee = ReDates.beginOfDate(new Date(now)).getTime();
        if (timeStampMillis >= wee) {
            // 'R' 24 小时制的时间，被格式化为 "%tH:%tM"
            return String.format("今天%tR", timeStampMillis);
        } else if (timeStampMillis >= wee - ReDates.MILLIS_PER_DAY) {
