 ...
 * 关于文件的工具集.
 * <p>
 * 主要是调用JDK自带的Files工具类，少量代码调用Guava ReFiles。 固定encoding为UTF8.
 * <p>
 * 1.文件读写
 * <p>
 * 2.文件及目录操作
...
public class ReFiles {
    ...
    // fileVisitor for file deletion on ReFiles.walkFileTree
    ...
    public static String toString(final File file) throws IOException {
        return com.google.common.io.Files.toString(file, ReCharsets.UTF_8);
    }
    ...
    public static List<String> toLines(final File file) throws IOException {
        return Files.readAllLines(file.toPath(), ReCharsets.UTF_8);
    }
    ...
    public static void write(final CharSequence data, final File file) throws IOException {
        ...
        try (BufferedWriter writer = Files.newBufferedWriter(file.toPath(), ReCharsets.UTF_8)) {
            writer.append(data);
        }
    }
    ...
    public static BufferedReader asBufferedReader(Path path) throws IOException {
        Validate.notNull(path, "path is null");
        return Files.newBufferedReader(path, ReCharsets.UTF_8);
    }
    ...
    public static BufferedWriter asBufferedWriter(String fileName) throws IOException {
        Validate.notBlank(fileName, "filename is blank");
        return Files.newBufferedWriter(getPath(fileName), ReCharsets.UTF_8);
    }
    ...
    public static BufferedWriter asBufferedWriter(Path path) throws IOException {
        Validate.notNull(path, "path is null");
        return Files.newBufferedWriter(path, ReCharsets.UTF_8);
    }
    ...
    /**
     * 文件复制. @see {@link Files#copy(Path, Path, CopyOption...)}
     *
     * @param from 如果为null，或文件不存在或者是目录，，抛出异常
     * @param to   如果to为null，或文件存在但是一个目录，抛出异常
     */
    public static void copyFile(@NotNull Path from, @NotNull Path to) throws IOException {
        ...
        Validate.notNull(to);
        Validate.isTrue(!ReFiles.isDirExists(to), "%s is exist but it is a dir", to);
        Files.copy(from, to);
    }
    ...
    public static String getFileName(@NotNull String fullName) {
        ...
        int last = fullName.lastIndexOf(RePlatforms.FILE_PATH_SEPARATOR_CHAR);
